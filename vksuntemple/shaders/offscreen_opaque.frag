...
void main() {
    ...
    switch (shade.visualisationMode) {
        case pbrMode:
            colour = pbrColour(fragNormal_wcs);
            break;
        case normalMode:
            colour = fragNormal_wcs;
            break;
        case viewDirectionMode:
    // Fragment -> Camera
            colour = normalize(shade.cameraPosition_wcs - position_wcs);
            break;
        case lightDirectionMode:
    // Fragment -> Light
            colour = normalize(shade.light.position_wcs - position_wcs);
            break;
        case roughnessMode:
    // Expands to greyscale colour [r, r, r]
            colour = texture(roughness, uv).rgb;
            break;
        case metalnessMode:
    // Expands to greyscale colour [m, m, m]
            colour = texture(metalness, uv).rgb;
            break;
        case normalMapMode:
            colour = texture(normalMap, uv).rgb;
            break;
        case baseMode:
            colour = texture(baseColour, uv).rgb * mesh.colour;
            break;
        default:
            break;
    }
